...
export function AIVoiceHub({ userName = "Guest", onNavigate, needsKYC, userRole, isAdmin }: AIVoiceHubProps) {
  ...
  const quickActions = [
    ...
    ...(userRole === 'owner' ? [{ id: 'owner-dashboard', icon: Home, label: t.quickActions.ownerDashboard || 'لوحة المالك', route: '/owner-dashboard' }] : []),
    ...(userRole === 'handyman' ? [{ id: 'handyman-dashboard', icon: Wrench, label: t.quickActions.handymanDashboard || 'لوحة الحرفي', route: '/handyman-dashboard' }] : []),
    ...
    ...(isAdmin ? [{ id: 'admin', icon: Shield, label: t.quickActions.admin || 'لوحة الإدارة', route: '/admin' }] : []),
  ];
